...
import { URL_REG } from '@/muya-v2/config'
import HtmlToMarkdown from '@/muya-v2/jsonState/htmlToMarkdown'
import MarkdownToState from '@/muya-v2/jsonState/markdownToState'
import { checkCopyType, normalizePastedHTML } from '@/muya-v2/utils/paste'
...
export default {
  async pasteHandler (event) {
    ...
    if (/html|text/.test(copyType)) {
      let markdown = copyType === 'html' ? new HtmlToMarkdown({ bulletListMarker }).generate(html) : text

      if (/\n\n/.test(markdown)) {
        if (start.offset !== end.offset) {
          contentBlock.text = content.substring(0, start.offset) + content.substring(end.offset)
          contentBlock.update()
        }
        ...
        contentBlock.text = content.substring(0, start.offset) + markdown + content.substring(end.offset)

...
import { PREVIEW_DOMPURIFY_CONFIG } from '@/muya-v2/config'
import { sanitize } from '@/muya-v2/utils'
import loadRenderer from '@/muya-v2/utils/diagram'
...
const renderDiagram = async ({ type, code, target, sequenceTheme, vegaTheme, mermaidTheme }) => {
  ...
  if (type === 'flowchart' || type === 'sequence') {
    ...
    // 验证code是否合法
    render.parse(code)
    // mermaid 缓存会缓存渲染过的节点， 不会二次渲染，导致渲染无法重新触发

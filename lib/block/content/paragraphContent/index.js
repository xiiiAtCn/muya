...
import Format from '@/muya-v2/block/base/format'
import ScrollPage from '@/muya-v2/block/scrollPage'
import { HTML_TAGS, VOID_HTML_TAGS } from '@/muya-v2/config'
import { isLengthEven, mixins } from '@/muya-v2/utils'
...
class ParagraphContent extends Format {
  ...
  enterConvert (event) {
    ...
    } else if (tableMatch && isLengthEven(tableMatch[1]) && isLengthEven(tableMatch[2])) {
      const tableHeader = parseTableHeader(this.text)
      const tableBlock = ScrollPage.loadBlock('table').createWithHeader(this.muya, tableHeader)

      this.parent.replaceWith(tableBlock)

      // Set cursor at the first cell of second row.
      tableBlock.firstChild.find(1).firstContentInDescendant().setCursor(0, 0, true)

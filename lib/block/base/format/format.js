...
import { FORMAT_MARKER_MAP, FORMAT_TYPES } from '@/muya-v2/config'
import { generator, tokenizer } from '@/muya-v2/inlineRenderer/lexer'
import Selection from '@/muya-v2/selection'
import { getImageInfo } from '@/muya-v2/utils/image'
...
export default {
  ...
  format (type, { start, end } = this.getCursor()) {
    ...
    const [currentFormats, currentNeightbors] = [formats, neighbors].map(item =>
      item
        .filter(format => {

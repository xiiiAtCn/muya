...
import { PARAGRAPH_STATE, THEMATIC_BREAK_STATE } from '@/muya-v2/config'
...
export default {
  ...
  convertToAtxHeading (atxHeading) {
    ...
    if (this.parent.blockName === 'atx-heading' && this.parent.meta.level === level) {
      return
    }
  ...
  convertToSetextHeading (setextHeading) {
    ...
    if (this.parent.blockName === 'setext-heading' && this.parent.meta.level === level) {
      return
    }
  ...
  convertToParagraph (force = false) {
    if (!force && (this.parent.blockName === 'setext-heading' || this.parent.blockName === 'paragraph')) {
      return
    }

...
import ScrollPage from '@/muya-v2/block'
import Clipboard from '@/muya-v2/clipboard'
import History from '@/muya-v2/history'
import InlineRenderer from '@/muya-v2/inlineRenderer'
import JSONState from '@/muya-v2/jsonState'
import Search from '@/muya-v2/search'
import Selection from '@/muya-v2/selection'
import { hasPick } from '@/muya-v2/utils'
...
class Editor {
  ...
  setMarkdown(markdown) {
    this.muya.options.markdown = markdown
    if (this.jsonState) {
    }
  }
  ...
  updateContents(operations, selection, source) {
    ...
    if (selection) {
      ...
      const cursorBlock = this.scrollPage.queryBlock(path)

      if (cursorBlock && cursorBlock.isContentBlock) {

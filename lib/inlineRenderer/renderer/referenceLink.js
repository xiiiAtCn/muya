...
import { snakeToCamel } from '@/muya-v2/utils'
import { sanitizeHyperlink } from '@/muya-v2/utils/url'

export default function referenceLink (h, cursor, block, token, outerClass) {
  const className = this.getClassName(outerClass, block, token, cursor)
  const labelClass = className === CLASS_NAMES.MU_GRAY ? CLASS_NAMES.MU_REFERENCE_LABEL : className

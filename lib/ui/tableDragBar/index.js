...
import { throttle } from '@/muya-v2/utils'
...
export const getIndex = (barType, cellBlock) => {
  ...
  return barType === 'bottom' ? row.offset(cellBlock) : table.firstChild.offset(row)
}
...
class TableDragBar extends BaseFloat {
  ...
  listen() {
    ...
    const handler = throttle(event => {
      ...
      if (!this.isDragTableBar && !hasTableCell(eles) && (hasTableCell(aboveEles) || hasTableCell(rightEles))) {
        const tableCellEle = [...aboveEles, ...rightEles].find(
  ...
  switchTableData = () => {
    ...
    if (table.active) {
      const { block, start, end } = this.muya.editor.selection
      const { rowOffset, columnOffset } = block.closestBlock('table.cell')
      ...
      if (barType === 'bottom') {
        ...
        } else if (columnOffset >= Math.min(index, curIndex) && columnOffset <= Math.max(index, curIndex)) {
        ...
        } else if (rowOffset >= Math.min(index, curIndex) && rowOffset <= Math.max(index, curIndex)) {
    ...
    if (cursorRowOffset !== null && cursorColumnOffset !== null) {
      const cursorBlock = newTable.firstChild.find(cursorRowOffset).find(cursorColumnOffset).firstContentInDescendant()

...
import deleteIcon from '@/muya-v2/assets/icons/delete/2.png'
import newIcon from '@/muya-v2/assets/icons/paragraph/2.png'
import { isOsx } from '@/muya-v2/config'
import { MENU_CONFIG } from '@/muya-v2/ui/quickInsert/config'

const ALL_MENU_CONFIG = MENU_CONFIG.reduce((acc, section) => [...acc, ...section.children], [])
...
export const canTurnIntoMenu = block => {
  ...
  switch (blockName) {
    case 'paragraph': {
      ...
      const PARAGRAPH_TURN_INTO_REG = /paragraph|atx-heading|block-quote|order-list|bullet-list|task-list/

      return ALL_MENU_CONFIG.filter(item => PARAGRAPH_TURN_INTO_REG.test(item.label))
    }
